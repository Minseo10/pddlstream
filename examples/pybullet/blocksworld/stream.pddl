(define (stream pr2-tamp)
  (:stream sample-table-pose
    :inputs (?o ?table)
    :domain (and (Stackable ?o ?table)(block ?o) (table ?table))
    :outputs (?p)
    :certified (and (Pose ?o ?p) (Supported ?o ?p ?table) (table-support ?o ?p ?table))
  )
  (:stream sample-block-pose
    :inputs (?o ?undero ?punder)
    :domain (and (Stackable ?o ?undero) (block ?o) (block ?undero) (AtPose ?undero ?punder))
    :outputs (?p)
    :certified (and (Pose ?o ?p) (Supported ?o ?p ?undero) (block-support ?o ?p ?undero ?punder))
  )
  (:stream sample-grasp
    :inputs (?o)
    :domain (and (Graspable ?o) (block ?o))
    :outputs (?g)
    :certified (Grasp ?o ?g)
  )
