...
from examples.pybullet.utils.pybullet_tools.utils import get_bodies, sample_placement, sample_placement_half, pairwise_collision, define_placement, \
    ...
    RED, GREEN, BLUE, BLACK, WHITE, BROWN, TAN, GREY, YELLOW, PINK, PURPLE
...
# 2) 블록 생성 함수 수정
def create_blocks_from_pddl(pddl_path, block_width=0.07, block_height=0.07):
    names = parse_all_block_names(pddl_path)
    blocks_by_color = {}
    for name in names:
        color = COLOR_CONST.get(name.lower())
        if color is None:
            raise ValueError(f"Unknown color: {name}")
        blocks_by_color[name] = create_box(block_width, block_width,
                                           block_height, color=color)
    return blocks_by_color


def num_from_pddl(problem_pddl_path):
    # "problem" 바로 뒤에 오는 숫자(들)를 정규표현식으로 추출
    match = re.search(r'problem(\d+)', problem_pddl_path)
    if match:
...
def sample_placements_from_problem(body_surfaces, table, obstacles=None, max_attempts=50, min_distances={}, **define_kwargs):
    """
...
def packed(arm='left', grasp_type='top', problem_pddl_path=None):
    ...
    blocks_by_color = create_blocks_from_pddl(problem_pddl_path)
    ...
    return Problem(robot=pr2, movable=blocks, arms=[arm], grasp_types=[grasp_type], surfaces=surfaces,
                   ...
                   goal_on=goal_on, base_limits=base_limits), initial_surfaces
